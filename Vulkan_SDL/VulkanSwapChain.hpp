...
public:
	...
	*/
	...
	)
	{
		...
		if (err != VK_SUCCESS) {
			throw std::runtime_error("Could not create surface!. Line: " + __LINE__ + ((std::string)" File: ") + __FILE__);
		}
		...
		{
			throw std::runtime_error("Could not find a graphics and/or presenting queue. Line: " + __LINE__ + ((std::string)" File: ") + __FILE__);
		}

		// todo : Add support for separate graphics and presenting queue
		if (graphicsQueueNodeIndex != presentQueueNodeIndex)
		{
			throw std::runtime_error("Separate graphics and presenting queues are not supported yet. Line: " + __LINE__ + ((std::string)" File: ") + __FILE__);
		}
		...
		if(fpGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, surface, &formatCount, NULL)!=VK_SUCCESS){
			throw std::runtime_error("Could not get supported surface formats. Line: " + __LINE__ + ((std::string)" File: ") + __FILE__);
	...
	*/
	void create(uint32_t *width, uint32_t *height, bool vsync = false)
	{
		...
		if (fpGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice, surface, &surfCaps)!=VK_SUCCESS) {
			throw std::runtime_error("Could not get physical device surface formats. Line: " + __LINE__ + ((std::string)" File: ") + __FILE__);
		}

		// Get available present modes
		uint32_t presentModeCount;
		if (fpGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, surface, &presentModeCount, NULL)!=VK_SUCCESS) {
			throw std::runtime_error("Could not get available present modes. Line: " + __LINE__ + ((std::string)" File: ") + __FILE__);
		}
		...
		if (fpCreateSwapchainKHR(device, &swapchainCI, nullptr, &swapChain)!=VK_SUCCESS) {
			throw std::runtime_error("Could not create swap chain. Line: " + __LINE__ + ((std::string)" File: ") + __FILE__);
		...
		if (fpGetSwapchainImagesKHR(device, swapChain, &imageCount, NULL)!=VK_SUCCESS) {
			throw std::runtime_error("Could not get swap chain images. Line: " + __LINE__ + ((std::string)" File: ") + __FILE__);
		}
		...
		for (uint32_t i = 0; i < imageCount; i++)
		{
			...
			if(vkCreateImageView(device, &colorAttachmentView, nullptr, &buffers[i].view)!=VK_SUCCESS) {
				throw std::runtime_error("Could not create image views. Line: " + __LINE__ + ((std::string)" File: ") + __FILE__);
			}
		}
	}
